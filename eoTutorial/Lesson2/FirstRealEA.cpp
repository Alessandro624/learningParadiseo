...
void main_function(int /*argc*/, char ** /*argv*/)
{
  ...
  const unsigned int VEC_SIZE = 16; // Number of object variables in genotypes
  ...
  eoUniformGenerator<double> uGen1(-2.0, -1.0);
  eoInitFixedLength<Indi> random1(VEC_SIZE, uGen1);
  eoUniformGenerator<double> uGen2(1.0, 2.0);
  eoInitFixedLength<Indi> random2(VEC_SIZE, uGen2);
  // Initialization of the population
  eoPop<Indi> pop(POP_SIZE / 2, random1);
  pop.append(POP_SIZE, random2);
  ...
  // stop when fitness reaches a target (here 0)
