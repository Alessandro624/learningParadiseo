...
// Updater for applying local search at checkpoints
class LocalSearchUpdater : public eoUpdater
{
private:
    eoPop<Queen> &pop;
    moFirstImprHC<shiftNeighbor> &localSearch;

public:
    LocalSearchUpdater(eoPop<Queen> &_pop, moFirstImprHC<shiftNeighbor> &_localSearch)
        : pop(_pop), localSearch(_localSearch) {}

    void operator()() override
    {
        for (auto &indiv : pop)
        {
            localSearch(indiv); // Apply local search on each individual
        }
    }
};
...
void main_function(int argc, char **argv)
{
    ...
    // updater
    LocalSearchUpdater updater(pop, hc);

    // continuator
    eoGenContinue<Queen> genCont(50);

    // checkpoint
    eoCheckPoint<Queen> checkpoint(genCont);
    updater.addTo(checkpoint);
    ...
    // Mutation operator (instead of using HC here)
    eoSwapMutation<Queen> mutation;

    // transform operator (the hill climber replace the mutation operator)
    eoSGATransform<Queen> transform(cross, 0.3, mutation, 0.7);

    // replacement
    eoGenerationalReplacement<Queen> repl;

    // easyEA
    eoEasyEA<Queen> hybridAlgo(checkpoint, fullEval, select, transform, repl);
